    ...
    for subitem in os.listdir(path_mescalina+'/'+item):       
        for subsubitem in os.listdir(path_mescalina+'/'+item+'/'+subitem):
            if 'L-S' == subitem:
                ...
                spect1_mescalina.append(spect1)
...
spect1_mescalina = np.reshape(spect1_mescalina,(len(files_mescalina),len(spect1)))
...
mescalinacontext = np.column_stack((spect1_mescalina,context_categories))
np.savetxt('mescalina_context_spect.csv', mescalinacontext, delimiter=',',fmt="%s")
del mescalinacontext
print('generating individ file')
mescalinaindivid = np.column_stack((spect1_mescalina,individ_categories))
np.savetxt('mescalina_individ_spect.csv', mescalinaindivid, delimiter=',',fmt="%s")
